...
public class DonationSPController {

    BasicBSL donationsBSl = new BasicBSL();
    @PostMapping
    public ResponseEntity<Object> donationPayment (@RequestBody DonationsForm form) {
        if (CustomerController.currentCustomer == null)
            return new ResponseEntity<>("Login as a customer", HttpStatus.UNAUTHORIZED);

        Transaction createdTransaction = donationsBSl.invokeDonationsSP(form);
